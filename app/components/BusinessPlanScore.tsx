...
const getScoreLabel = (score: number) => {
  if (score >= 95) return { emoji: '🏆', label: 'Top 1%', cta: 'VIP fast track – we\'ll ping VC scouts today.' };
  if (score >= 85) return { emoji: '🚀', label: 'Deal-ready', cta: 'Click for warm intro to our investor network.' };
  if (score >= 75) return { emoji: '⭐', label: 'Investable with guidance', cta: 'We\'ll match you with angel/ALMI – let\'s polish the pitch first.' };
  if (score >= 50) return { emoji: '⚙️', label: 'Potential, needs work', cta: 'Get a 30-day action plan + coach.' };
  return { emoji: '🚧', label: 'Under construction', cta: 'Here are the top 3 gaps to address – book a workshop.' };
};
