...
def create_swot_diagram(swot_text):
    """Creates a visual SWOT diagram from text with optimized memory management"""
    ...
    colors = {
        ...
        'Weaknesses': '#F44336',    # Red
        'Opportunities': '#2196F3',  # Blue
        'Threats': '#FF9800'        # Orange
    }
    
    # Titles and content for each quadrant
    sections_order = ['Strengths', 'Weaknesses', 'Opportunities', 'Threats']
    ...
    for (section, pos) in zip(sections_order, positions):
        ...
        ax.set_facecolor(colors.get(section, '#EEEEEE') + '22')  # Add transparency
        
        # Set title
        ax.set_title(section, fontsize=14, fontweight='bold', color=colors.get(section, '#333333'))
        
        # Add content
        content = sections.get(section, ['No information available'])
        y_pos = 0.9
        for item in content[:5]:  # Limit to max 5 points for better readability and performance
            if len(item) > 55:
                item = item[:52] + '...'  # Limit text length for display and performance
    ...
    # Save figure to buffer with limited resolution for better performance
    buf = io.BytesIO()
