...
def create_pdf_report(data, swot_text=None, swot_image=None, manifest=None, logo_url=None):
    """Creates a PDF report with all business plan information"""
    pdf = FPDF()
    ...
    info_items = [
        ("City", data.get('stad', 'N/A')),
        ("Target Group", data.get('malgrupp', 'N/A')),
        ("Product Range", data.get('produktutbud', 'N/A')),
        ("Strategy", data.get('strategi', 'N/A')),
        ("Timeline", data.get('tidsplan', 'N/A')),
    ...
    if 'affarsplan' in data:
        pdf.add_page()
        pdf.set_font("Arial", "B", 16)
        pdf.cell(190, 10, "Business Plan", ln=True)
        pdf.ln(5)
        ...
        for paragraph in paragraphs:
            # Check if it's a heading (assume headings are short lines ending with colon)
