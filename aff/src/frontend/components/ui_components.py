...
def progress_bar(stage):
    """Shows a progress bar based on the user's current step"""
    stages = ['intro', 'basic_info', 'deep_dive', 'financial', 'business_plan']
    labels = ["Start", "Info", "Analysis", "Finance", "Plan"]
    # Calculate percentage for progress bar
    progress_val = (stages.index(stage) + 1) / len(stages)
    
    # Use Streamlit's built-in progress bar with custom height
    ...
    # Show step text with custom HTML to match design
    st.markdown("""
